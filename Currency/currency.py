...
async def momentarily_currency_rate(user_language):
  ...
  currency_names = [
    languages[user_language]['dollar_currency'],
    languages[user_language]['euro_currency'],
    languages[user_language]['pound_currency'],
    languages[user_language]['lir_currency'],
    languages[user_language]['swiss_franc_currency'],
  ...
  async with aiohttp.ClientSession() as session:
    for currency in currencies_list:
      tasks.append(
        asyncio.ensure_future(current_currency_rate(session, currency)))
  ...
  for currency in currencies_dictionary:
    if '-' in currencies_dictionary[currency]:
      currencies_dictionary[currency]['?'] = currencies_dictionary[
        currency].pop('-')
